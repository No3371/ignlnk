# Red Team (Pass 2): ignk MVP Implementation Plan (rev 2)

> **Created:** 2026-02-11 | **Lead:** Claude (agent) | **Mode:** Attack + Skeptic + Forensic
> **Subject:** `20260211-ignk-mvp-plan.md` (rev 2 — post first red team)
> **Related:** `20260211-ignk-mvp-plan-redteam.md`, `20260211-ignk-mvp-plan-redteam-review.md`, `20260211-ignk-cli-tool-proposal.md`
...
**Top Vulnerabilities:**
1. **doublestar != gitignore** — `*.pem` in `.ignkfiles` silently won't match `config/ssl/server.pem` (users expect gitignore semantics; doublestar doesn't provide them)
...
3. **Manifest lock held for entire lock-all** — blocks all other ignk commands for the duration, no timeout escape
...
|------|---------------------|
| End Users | `.ignkfiles` patterns don't behave like `.gitignore` despite claiming to; lock-all hangs block status/list |
...
- Reality: `~/.ignk/vault/` created with default umask permissions; no explicit 0700. Other users on multi-user systems can read secrets.
...
**Attack Vector:** User writes `.ignkfiles`:
```
*.pem
*.env
node_modules
...
- **Security:** Files the user believes are protected remain exposed. The `.ignkfiles` documentation says "like .gitignore" but the behavior differs in the most common use case.

**Blast Radius:** Every `.ignkfiles` pattern without an explicit `**/` prefix. This is the default way users write patterns (copied from their `.gitignore` experience).
...
**Bug 1: ForgetFile double-saves.** If user runs `ignk forget a.txt b.txt`, ForgetFile saves after forgetting `a.txt`, then again after `b.txt`, then cmd/forget.go saves again. Three writes instead of one. Functional but wasteful and confusing.

**Bug 2: Partial failure orphans files with Pattern A.** If `ignk lock a.txt b.txt c.txt` succeeds for `a.txt` and `b.txt` but fails on `c.txt`:
...
- If the command exits without saving: `a.txt` and `b.txt` are locked on disk but NOT in the manifest. Orphaned. `ignk status` doesn't show them. `ignk forget a.txt` fails ("not managed"). User has no way to recover without manually restoring from vault.
...
**Attack Vector:** User runs `ignk lock-all` in a project with 50 files matching `.ignkfiles` patterns. Each file requires: hash (~200ms for 1MB) + copy (~200ms) + verify hash (~200ms) + atomic placeholder write (~10ms). Per file: ~600ms. Total: ~30 seconds.
...
**Role-Specific Impact:**
- **End Users:** `ignk status` becomes unresponsive during lock-all/unlock-all. User thinks the tool is broken.
...
- **Option C (timeout):** Add a lock acquisition timeout (5s default) to all commands. If lock can't be acquired, print "another ignk operation is in progress, try again shortly" instead of hanging indefinitely.
...
> "Stale lock detection: if `.ignk/manifest.lock` is >5 minutes old and the PID that created it is gone, clean it up"
...
- On timeout, print: "Could not acquire lock. Another ignk operation may be running. If no other operation is active, delete `.ignk/manifest.lock` and retry."
...
**Role-Specific Impact:**
- **End Users:** `ignk init` says symlinks are supported (test passed in temp). `ignk unlock` fails later when trying to create symlink in the actual project directory. Confusing — "but init said it was fine!"
...
**Remediation:** Change CheckSymlinkSupport to create the test symlink in the `.ignk/` directory (which is on the same filesystem as the project). Test in the filesystem that will actually be used. Clean up the test symlink afterward.
...
**Remediation:** Explicitly create `~/.ignk/` and all vault directories with `0700` permissions:
```go
...
```
> type config/secrets.yaml    ← forward slash from ignk output
```

On cmd.exe, forward slashes may not work. On PowerShell, they do. Inconsistent experience.

**Role-Specific Impact:**
- **End Users:** Confusion on Windows. Copy-paste from ignk output may not work in cmd.exe.
- **Integrators:** Scripts parsing ignk output get forward-slash paths regardless of OS.
...
**Attack Vector:** User creates `.ignkfiles` with `**/*.yaml`. Runs `ignk lock-all`. Discovers it locked 47 YAML files including ones they didn't intend (test fixtures, CI configs, documentation examples). Undoing requires `ignk forget` on each.
...
**Attack Vector:** On Linux, `~/.ignk/` violates the XDG Base Directory Specification. Tools like chezmoi, pass, and docker-credential-helpers use `$XDG_DATA_HOME` (~/.local/share/). Users with constrained home directories or NFS-mounted homes may not expect large vault data in `~/`.
...
**Remediation:** For MVP, document `~/.ignk/` as the default location. Future: respect `$IGNK_HOME` environment variable for override. XDG compliance can come post-MVP.
...
**Counter-Evidence:** Lock-all with 50 files takes ~30 seconds of lock hold time. During this window, `ignk status` hangs.
...
**Trigger:** CI matrix with 4 shards, each running `ignk lock-all` before tests.
...
**Role Experience:** `ignk unlock large.db` takes 2-5 seconds for the hash verification. No progress output (size gate from first red team only applies to LockFile, not UnlockFile). User thinks it's hanging.
...
**Omissions per role:**
- **End Users:** No mention that doublestar `*` behaves differently from gitignore `*`. The `.ignkfiles` documentation will mislead users who copy patterns from `.gitignore`.
...
6. **Test symlinks in project filesystem** (affects: End Users) → CheckSymlinkSupport should create test symlink in `.ignk/`, not `os.TempDir()`.
...
12. **Document vault location and override** (affects: Operators) → `~/.ignk/` is default. Document `$IGNK_HOME` for future override.
...
**No-Go If:**
- [ ] `.ignkfiles` ships with doublestar raw semantics and "like .gitignore" documentation — same class of silent false-security as the original filepath.Match bug
