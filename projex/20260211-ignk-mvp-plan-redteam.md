# Red Team: ignk MVP Implementation Plan

> **Created:** 2026-02-11 | **Lead:** Claude (agent) | **Mode:** Attack + Forensic
> **Subject:** `20260211-ignk-mvp-plan.md` | **Related:** `20260211-ignk-cli-tool-proposal.md`
> **Reviewed:** 2026-02-11 — `20260211-ignk-mvp-plan-redteam-review.md`
...
2. **`filepath.Match` cannot implement `.ignkfiles` patterns** — `**` globstar silently fails, giving false sense of security
...
|------|-------------|-------------|---------------------|
| End Users (developers) | Secrets stay hidden from AI agents | Must remember to re-lock; cryptic errors on Windows | Symlinks "just work"; `.ignkfiles` patterns behave like `.gitignore` |
...
- Assumptions: `.ignkfiles` patterns match recursively; Windows works out of the box
...
**Attack Vector:** Two terminals run `ignk lock file1.txt` and `ignk lock file2.txt` simultaneously. Both read the manifest, both add their entry, both write. Second write overwrites first. `file1.txt` is now in the vault but absent from the manifest — orphaned and invisible.

**Role-Specific Impact:**
- **End Users:** Silent data loss. `ignk status` doesn't show file1. `ignk forget file1.txt` fails ("not managed"). Vault accumulates orphans. User may not notice until they need the file.
- **Operators:** CI jobs running `ignk unlock` in parallel (matrix builds, parallel test shards) corrupt manifest. Flaky, non-reproducible failures.
...
- `.ignk/manifest.lock` for project operations; `~/.ignk/index.lock` for global operations
...
### Finding 2: `filepath.Match` Cannot Implement `.ignkfiles` Patterns

**Severity:** Critical | **Likelihood:** High

**Affects Roles:** End Users, Security

**Attack Vector:** User creates `.ignkfiles` with `**/*.pem` expecting all PEM files in all subdirectories to be protected. `filepath.Match` does not support `**` globstar — pattern silently matches nothing. User runs `ignk lock-all`, sees "locked 0 new files", and assumes no PEM files exist. Meanwhile `config/ssl/server.pem` sits unprotected, fully readable by any AI agent.
...
**Blast Radius:** Every `.ignkfiles` pattern using `**`, negation (`!`), or anchoring (`/`) fails. These are the patterns users will copy directly from their `.gitignore` experience.
...
- **Option C:** Defer `.ignkfiles` entirely to post-MVP — ship with explicit paths only. Better than shipping a broken feature that creates false confidence.
...
**Attack Vector:** Developer on Windows runs `ignk lock config\secrets.yaml`. Manifest stores `"config\\secrets.yaml"`. Committed via git. Developer on macOS runs `ignk unlock config/secrets.yaml`. Looks up `"config/secrets.yaml"` — key not found. "File not managed" error.
...
- **Case sensitivity:** Windows filesystem is case-insensitive. `ignk lock Config.yaml` then `ignk unlock config.yaml` — key mismatch with exact string lookup.
...
- **Path escape:** `ignk lock ../outside-project/secret.txt` — `filepath.Rel` produces `../outside-project/secret.txt`, which escapes the project root. No validation prevents this.
...
**Attack Vector:** User installs ignk on Windows 10. Runs `ignk init`, `ignk lock .env` — works fine (no symlinks needed). Runs `ignk unlock .env` — fails with `"A required privilege is not held by the client"`. User has no idea what this means or how to fix it.
...
**Remediation:**
- **Immediate:** Detect symlink capability at `ignk init` — attempt to create and remove a test symlink in `.ignk/`. Fail with clear message: "Windows Developer Mode required. Enable in Settings > Update & Security > For Developers, or run as Administrator."
- **Immediate:** Detect at `ignk unlock` before any file operations — fail early with actionable error.
- **Future:** Implement `--no-symlink` copy-swap fallback (currently out of scope, but its absence makes ignk unusable for a large Windows audience).
...
**Attack Vector:** User runs `ignk lock database.db` on a 2GB SQLite file. SHA-256 hash + copy + verification hash = 3 full reads of 2GB each. Tool appears to hang for 30+ seconds with no output. User hits Ctrl+C — original file already deleted, vault copy incomplete. Data lost.

Alternatively: `.ignkfiles` matches `*.db` in a project with 50 database files totaling 20GB. `ignk lock-all` runs for minutes, pegs CPU at 100%, fills disk with vault copies.
...
```
$ ls -la .env
.env -> C:\Users\alice\.ignk\vault\a1b2c3d4\.env
```

Any agent running `ls -la`, `readlink`, or inspecting file metadata sees the full vault path including UID. From there, `ls ~/.ignk/vault/a1b2c3d4/` reveals every file in the vault — even ones still locked.
...
### End Users: "`.ignkfiles` works like `.gitignore`"
...
**Counter-Evidence:** `os.Readlink()` or `ls -la` on any unlocked file reveals `~/.ignk/vault/<uid>/`.
...
**Role Experience:** User runs `ignk unlock .env` — integrity check detects non-placeholder content. But what happens? Plan says "warn if modified" but doesn't specify recovery. User's vault copy is still safe, but the agent-generated `.env` is lost when unlock overwrites it.
...
**Trigger:** Project directory moved (e.g., `mv ~/old-project ~/new-project`). Proposal says "running any ignk command from the new location auto-updates the index" — but the plan doesn't implement this auto-update.
**Role Experience:** `ignk status` from new location fails with "not an ignk project" (walks up, doesn't find `.ignk/`... wait, `.ignk/` moved with the project). Actually: `FindProject` finds `.ignk/` fine. But `ResolveVault` looks up the *new* project root in the index — not found. Error: "project not registered."
**Recovery:** Difficult — user must figure out they need to re-register. Existing vault files under old UID are orphaned.
**Mitigation:** If `FindProject` succeeds but `ResolveVault` fails, search index by UID from vault (but UID isn't stored in-project by design). Alternative: store a non-secret project identifier in `.ignk/` that can be matched against the index.

### End Users: Lock File That's Already a Symlink
**Trigger:** User has a symlink in their project (not from ignk) and runs `ignk lock link.txt`.
...
**Omissions per role:**
- **End Users:** No mention of what happens if vault disk fills up mid-lock. No mention of backup strategy for the vault. No mention of how to recover if `~/.ignk/` is accidentally deleted.
...
3. **Replace `filepath.Match` or defer `.ignkfiles`** (affects: End Users, Security) → Use `github.com/bmatcuk/doublestar/v4` for glob matching, OR defer `.ignkfiles` to post-MVP → Verify with `**/*.pem` pattern matching `config/ssl/server.pem`
4. **Windows symlink detection** (affects: End Users, Operators) → Test symlink capability at `ignk init` and `ignk unlock`; fail early with actionable message → Verify on Windows without Developer Mode
5. **Path validation** (affects: Security, Integrators) → Reject `..` traversal and absolute paths in `RelPath` → Verify `ignk lock ../outside/file` is rejected
...
12. **Vault disk usage** (affects: Operators) → Track size; add `ignk vault-size` in future
...
**Per-Role Readiness:**
- **End Users:** Not Ready — `.ignkfiles` silently broken, Windows unlock fails, no concurrent safety
...
- **Security:** Ready with Caveats — threat model is honest, but symlink path leakage and `.ignkfiles` false security need addressing
...
- [ ] `filepath.Match` replaced with doublestar OR `.ignkfiles` deferred (for End Users, Security)
...
**No-Go If:**
- [ ] `.ignkfiles` ships with `filepath.Match` — creates false security (impacts End Users, Security)
